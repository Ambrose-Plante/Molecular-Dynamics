...
if checkpoint_load[-1] != "/":
    checkpoint_load = checkpoint_load + "/"
if checkpoint_save[-1] != "/":
...
if type(class1_data_files) != list:
    class1_data_files = [class1_data_files]
if type(class2_data_files) != list:
    class2_data_files = [class2_data_files]
if type(class3_data_files) != list:
    class3_data_files = [class3_data_files]
######################################

if not os.path.exists(checkpoint_save):
...
X_train_id, X_test_id, Y_train, Y_test = train_test_split(
    total_id, labels, test_size=0.20, random_state=42
)
# random_state makes sure you keep the same samples in each so that next time not in valid
# split of valid from train
X_train_id, X_valid_id, Y_train, Y_valid = train_test_split(
    X_train_id, Y_train, test_size=0.30, random_state=66
)
...
norm_parameters = np.concatenate((train_datagen.mean, train_datagen.std)).reshape(
    [2, -1]
)
...
checkpointer_bvl = ModelCheckpoint(
    filepath=checkpoint_save + "weights_best_val_loss.hdf5",
    monitor="val_loss",
    verbose=1,
...
checkpointer_new = ModelCheckpoint(
    filepath=checkpoint_save + "weights_newest.hdf5",
    verbose=1,
    save_weights_only=True,
...
model.load_weights(
    checkpoint_load + "weights_best_val_loss.hdf5"
)  # load the weight with best performance from last step to continue training
print(model.summary())
...
history = model.fit(
    ...
    callbacks=[checkpointer_bvl, checkpointer_new, lrsc],
    shuffle=True,
)
...
accuracy = (
    100
    * np.sum(np.argmax(train_result, axis=1) == np.argmax(Y_train, axis=1))
    / Y_train.shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class1_train_result, axis=1) == np.argmax(Y_train[class1_ind], axis=1)
    )
    / Y_train[class1_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class2_train_result, axis=1) == np.argmax(Y_train[class2_ind], axis=1)
    )
    / Y_train[class2_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class3_train_result, axis=1) == np.argmax(Y_train[class3_ind], axis=1)
    )
    / Y_train[class3_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(np.argmax(valid_result, axis=1) == np.argmax(Y_valid, axis=1))
    / Y_valid.shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class1_valid_result, axis=1) == np.argmax(Y_valid[class1_ind], axis=1)
    )
    / Y_valid[class1_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class2_valid_result, axis=1) == np.argmax(Y_valid[class2_ind], axis=1)
    )
    / Y_valid[class2_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class3_valid_result, axis=1) == np.argmax(Y_valid[class3_ind], axis=1)
    )
    / Y_valid[class3_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(np.argmax(test_result, axis=1) == np.argmax(Y_test, axis=1))
    / Y_test.shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class1_test_result, axis=1) == np.argmax(Y_test[class1_ind], axis=1)
    )
    / Y_test[class1_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class2_test_result, axis=1) == np.argmax(Y_test[class2_ind], axis=1)
    )
    / Y_test[class2_ind].shape[0]
)
...
accuracy = (
    100
    * np.sum(
        np.argmax(class3_test_result, axis=1) == np.argmax(Y_test[class3_ind], axis=1)
    )
    / Y_test[class3_ind].shape[0]
)
