    ...
    if image:
        ...
        for y in range(traj.shape[0]):
            for i in range(3):
                traj[y, :, i] = (
                    255.0
                    * (traj[y, :, i] - extrema[0, i])
                    / (extrema[1, i] - extrema[0, i])
                )

    traj = np.reshape(
        np.transpose(traj, (1, 0, 2)),
        (img_rows, img_cols, traj.shape[0], traj.shape[2]),
    )
...
def preprocess(traj, labels, dim_diff, num_classes):
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(
        traj, labels, test_size=0.20, random_state=42
    )
    X_train, X_valid, Y_train, Y_valid = train_test_split(
        X_train, Y_train, test_size=0.30, random_state=66
    )
