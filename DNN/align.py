...
dist_mat = np.array(
    [
        np.linalg.norm((temp[:, pair[0], :] - temp[:, pair[1], :]), axis=1)
        for pair in pairs
    ]
)
...
for i in Calphas:
    stds.append(
        np.std(
            dist_mat[np.where((pairs[:, 1] == i) | (pairs[:, 0] == i))[0]], axis=1
        ).mean()
    )
...
aligner = align.AlignTraj(
    mobile, ref, select="name CA", weights=(1 / stds**2), filename=out_dcd
).run()  # increase power to make sharper, decrease to make flatter
